...
@testset "Attestation Negative/Edge Cases" begin
    ...
    @test_throws Exception verify_attestation_object(b64_attobj, cdj_b)
    ...
    @test_throws Exception verify_attestation_object(b64_attobj2, cdj_b)
    ...
    @test_throws Exception verify_attestation_object(b64_attobj3, cdj_b)
    ...
    @test_throws Exception verify_attestation_object(b64_attobj7, cdj_b)

    # -- 8. attStmt empty
    attobj8 = Dict("fmt" => "packed", "authData" => authData,
        "attStmt" => Dict())
    b64_attobj8 = base64urlencode(CBOR.encode(attobj8))
    @test_throws Exception verify_attestation_object(b64_attobj8, cdj_b)

    # -- 9. Completely mismatched (missing all required fields)
    badobj = Dict("foo" => "bar")
    b64_bad = base64urlencode(CBOR.encode(badobj))
    @test_throws Exception verify_attestation_object(b64_bad, cdj_b)
end
