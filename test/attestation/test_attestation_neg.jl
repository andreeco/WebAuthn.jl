...
@testset "Attestation Negative/Edge Cases" begin
    ...
    x = rand(UInt8, 32); y = rand(UInt8, 32)
    cose_key = Dict(1=>2, 3=>-7, -1=>1, -2=>x, -3=>y)
    cbor_pk = CBOR.encode(cose_key)
    prefix = rand(UInt8, 37); aaguid = zeros(UInt8, 16); credlen = UInt8[0, 0]
    ...
    # alg mismatch: EdDSA signature, ES256 alg
    attStmt = Dict("sig"=>sig, "alg"=>-7)
    attobj = Dict("fmt"=>"packed", "authData"=>authData, "attStmt"=>attStmt)
    ...
    attobj2 = Dict("fmt"=>"packed","authData"=>authData,"attStmt"=>attStmt2)
    b64_attobj2 = base64urlencode(CBOR.encode(attobj2))
    @test !verify_attestation_object(b64_attobj2, cdj_b)

    # 3. AttStmt missing sig
    attStmt3 = Dict("alg"=>-7)
    attobj3 = Dict("fmt"=>"packed","authData"=>authData,"attStmt"=>attStmt3)
...
@testset "WebAuthn packed self-attestation alg/key mismatch" begin
    ...
    sl = Ref{Culonglong}()
    @test Sodium.crypto_sign_detached(sig, sl, msg, length(msg), sk) == 0
    # alg/key mismatch (Ed25519 sig, alg=-8, but EC key in COSE)
