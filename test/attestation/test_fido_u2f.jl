...
@testset "FIDO2/U2F Registration/Attestation" begin
    attestationObject = load_vector("vectors_fidou2f", "registration", 
    "attestationObject.cbor")
    clientDataJSON = load_vector("vectors_fidou2f", "registration", 
    "clientDataJSON.bin")
...
@testset "FIDO2/U2F Authentication/Assertion" begin
    ...
    ## Happy path
    authenticatorData = load_vector("vectors_fidou2f", "authentication", 
    "authenticatorData.bin")
    clientDataJSON_authn = load_vector("vectors_fidou2f", "authentication", 
    "clientDataJSON.bin")
    signature = load_vector("vectors_fidou2f", "authentication", 
    "signature.bin")
    @test verify_webauthn_signature(pubkey_pem,
        authenticatorData, clientDataJSON_authn, signature) == true

    ## Negative/edge: corrupted signature -- must fail
    sig2 = copy(signature)
    sig2[7] ⊻= 0x44
    @test !verify_webauthn_signature(pubkey_pem, authenticatorData,
        clientDataJSON_authn, sig2)

    ## Negative: corrupted challenge
    ...
    ## Negative: corrupted authenticatorData
