...
@testset "Attestation Happy Path Coverage" begin
    # NONE format
    att_none = load_vector("vectors_ec2_none", "registration",
        "attestationObject.cbor")
    cdj_none = load_vector("vectors_ec2_none", "registration",
        "clientDataJSON.bin")
    ...
    # PACKED/x5c
    att_x5c = load_vector("vectors_ec2_packed", "registration",
        "attestationObject.cbor")
    cdj_x5c = load_vector("vectors_ec2_packed", "registration",
        "clientDataJSON.bin")
    try
...
@testset "Attestation COSE_Key extraction" begin
    for v in (
        ...
        attobj = WebAuthn.parse_attestation_object(WebAuthn.base64urlencode(
            load_vector("vectors_ec2_none", "registration",
                "attestationObject.cbor")))
...
@testset "Registered attestation: Interop" begin
    v = Dict(
        "attestationObject" => load_vector("vectors_ec2_packed",
            "registration", "attestationObject.cbor"),
        "clientDataJSON" => load_vector("vectors_ec2_packed",
            "registration", "clientDataJSON.bin"),
        "signature" => load_vector("vectors_ec2_packed",
            "authentication", "signature.bin"),
        "authenticatorData" => load_vector("vectors_ec2_packed",
            "authentication", "authenticatorData.bin")
    )
...
@testset "verify_attestation_packed Ed25519 (OKP, alg -8) self-attestation" begin
    ...
    authData = vcat(rpIdHash, flags, signCount, aaguid, credIdLen, credId,
        cbor_pk)
...
@testset "tpm-ES256/registration" begin
    attobj = load_vector("vectors_tpm", "registration",
        "attestationObject.cbor")
...
@testset "packed-Ed448/registration" begin
    attobj = load_vector("vectors_ed25519_packed", "registration",
        "attestationObject.cbor")
    cdj = load_vector("vectors_ed25519_packed", "registration",
        "clientDataJSON.bin")
    ok = try
        WebAuthn.verify_attestation_object(WebAuthn.base64urlencode(attobj),
            cdj)
...
@testset "android-key-ES256/registration" begin
    attobj = load_vector("vectors_android", "registration",
        "attestationObject.cbor")
    cdj = load_vector("vectors_android", "registration",
        "clientDataJSON.bin")
    ok = try
        WebAuthn.verify_attestation_object(WebAuthn.base64urlencode(attobj),
            cdj)
...
@testset "apple-ES256/registration" begin
    attobj = load_vector("vectors_apple", "registration",
        "attestationObject.cbor")
    cdj = load_vector("vectors_apple", "registration",
        "clientDataJSON.bin")
    ok = try
        WebAuthn.verify_attestation_object(WebAuthn.base64urlencode(attobj),
            cdj)
...
@testset "packed-ES384/registration" begin
    attobj = load_vector("vectors_ec2_p384_packed", "registration",
        "attestationObject.cbor")
    cdj = load_vector("vectors_ec2_p384_packed", "registration",
        "clientDataJSON.bin")
    @test_broken WebAuthn.verify_attestation_object(
        WebAuthn.base64urlencode(attobj), cdj)
end