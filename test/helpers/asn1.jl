...
function _decode_one(data::Vector{UInt8}, pos::Int, depth::Int=0)
    ...
    if class == UNIVERSAL && isconst &&
       (tag == TAG_BIT_STRING || tag == TAG_OCTET_STRING)
        throw(ErrorException(
    ...
    if isconst || tag in (TAG_SEQUENCE, TAG_SET)
        ...
        if length(children) > 1
            encs = [AbstractSyntaxNotationOne.DER._encode_one(c) for c in children]
            if encs != sort(encs)
                throw(ErrorException("SET elements not in DER canonical order"))
            end
        end
    end
...
function der_to_asn1(node::DER.DERTree)
    # Unwrap context-specific or application EXPLICIT containers if found
    while node.class != DER.UNIVERSAL && !isempty(node.children)
