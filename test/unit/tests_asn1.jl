...
@testset "DER: Primitive and Constructed" begin
    @test der_to_asn1(
        AbstractSyntaxNotationOne.DER.decode(
            load_vector("keys", "int_42.der"))) == ASN1Integer(42)
    @test der_to_asn1(
        AbstractSyntaxNotationOne.DER.decode(
            load_vector("keys", "bool_true.der"))) == ASN1Boolean(true)
    @test der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "null.der"))) == ASN1Null()
    @test der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "oid_rsaEncryption.der")
    )) == ASN1OID([1, 2, 840, 113549, 1, 1, 1])
    @test der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "octetstring.der")
    )) == ASN1OctetString([0xde, 0xad, 0xbe, 0xef])
    @test der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "bitstring.der")
    )) == ASN1BitString(BitVector(
        [true, true, false, true, false, false, true, true]))  # 0xd3
    # PrintableString
    s = der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "string.der")))
    @test s isa ASN1String && s.str == "HelloJULIA"
    # SEQUENCE
    seq = der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "sequence.der")))
    @test seq isa ASN1Sequence
    # SET
    st = der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "set.der")))
    @test st isa ASN1Set
    # NESTED
    nest = der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "nested.der")))
...
@testset "DER: Malformed and Forbidden Encodings" begin
  ...
  #      load_vector("keys", "int_42_overlong.der"))
    @test_throws ASN1Error der_to_asn1(AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "int_42_overlong.der")))
    # Indefinite length encoding: forbidden in DER
    @test_throws ErrorException AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "octetstring_indef.der"))
    # Bad BIT STRING: unused bits out of bounds
    @test_throws ASN1Error der_to_asn1(
        AbstractSyntaxNotationOne.DER.decode(load_vector("keys", 
        "bitstring_badunused.der")))
    # Set out-of-order: check that it parses, but round-trip generates 
    # canonical encoding
    @test_throws ErrorException AbstractSyntaxNotationOne.DER.decode(
        load_vector("keys", "set_outoforder.der"))
end

@testset "DER: Public Key Structures" begin
    # RSA public key (SPKI)
    rsa_spki_der = load_vector("vectors_rsa_packed", "pubkey.der")
...
@testset "DER: Encode/Decode Idempotence" begin
    ...
    for name in [
        ...
        "x509_ca", "x509_ee"]
        original = load_vector("keys", name * ".der")
...
@testset "DER: All files parse or error, no segfault" begin
    for fname in readdir(VECTORS_DIR)
        endswith(fname, ".der") || continue
        data = read(joinpath(VECTORS_DIR, fname))
        try
            AbstractSyntaxNotationOne.DER.decode(data)
...
@testset "DERFirewall (OpenSSL C-layer) – all branches" begin
    ...
    @testset "Valid public keys are accepted (all types)" begin
        rsa_key = load_vector("vectors_rsa_packed", "pubkey.der")
        ec_key = load_vector("vectors_ec2_packed", "pubkey.der")
        ed_key = load_vector("vectors_ed25519_packed", "pubkey.der")
        # d2i_PUBKEY (RSA)
    ...
    @testset "Unsupported/exotic keys are never accepted" begin
        for exotic in ("secp384r1_spki", "x25519_spki", "ed448_spki")
            path = joinpath(VECTORS_DIR, exotic * ".der")
            if isfile(path)
                der = read(path)
    ...
    @testset "parse_rsa_publickey: valid and invalid" begin
        rsa_raw = load_vector("keys", "rsa_raw.der")
    ...
    @testset "Stress: 10000 rounds on ccall parse/free/cmp" begin
        keys = [
            load_vector("vectors_rsa_packed", "pubkey.der"),
            load_vector("vectors_ec2_packed", "pubkey.der"),
            load_vector("vectors_ed25519_packed", "pubkey.der")
        ]
        for i in 1:10000
            dat = keys[rand(1:end)]
            ptr = DERFIREWALL.parse_evp_pkey(dat)
